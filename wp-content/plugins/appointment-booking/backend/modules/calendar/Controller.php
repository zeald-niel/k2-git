...
class Controller extends Lib\Base\Controller
{
    ...
    public function executeGetStaffAppointments()
    {
        ...
        // Load special days.
        $special_days = array();
        foreach ( (array) Lib\Proxy\SpecialDays::getDataForAvailableTime( $staff_ids, $start_date, $end_date ) as $day ) {
            ...
            if ( $appointment['number_of_persons'] == $appointment['total_number_of_persons'] ) {
                ...
                foreach ( array( 'customer_name', 'customer_phone', 'customer_email' ) as $data_entry ) {
                    if ( $appointment[ $data_entry ] ) {
                        $description .= '<div>' . esc_html( $appointment[ $data_entry ] ) . '</div>';
                    }
                }

                Lib\Proxy\Shared::prepareCalendarAppointmentDescription( $description, $appointment );
                ...
                if ( $appointment['custom_fields'] != '[]' ) {
                    ...
                    foreach ( $ca->getCustomFields() as $custom_field ) {
                        $description .= sprintf( '<div>%s: %s</div>', wp_strip_all_tags( $custom_field['label'] ), nl2br( esc_html( $custom_field['value'] ) ) );
                    }
                }
                // Payment.
                ...
                // Status.
                $description .= sprintf( '<br/><div>%s: %s</div>', __( 'Status', 'bookly' ), Lib\Entities\CustomerAppointment::statusToString( $appointment['appointment_status'] ) );
                // Signed up & Capacity.
                if ( $appointment['max_capacity'] > 1 ) {
                    $description .= sprintf( '<br/><div>%s: %d</div>', __( 'Signed up', 'bookly' ), $appointment['total_number_of_persons'] );
                    $description .= sprintf( '<div>%s: %d</div>', __( 'Capacity',  'bookly' ),  $appointment['max_capacity'] );
                }
            } else {
                $description .= sprintf( '<div>%s: %d</div>', __( 'Signed up', 'bookly' ), $appointment['total_number_of_persons'] );
                $description .= sprintf( '<div>%s: %d</div>', __( 'Capacity',  'bookly' ), $appointment['max_capacity'] );
            }
