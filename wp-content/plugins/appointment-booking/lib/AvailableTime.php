...
class AvailableTime
{
    ...
    public function load( array $params = array() )
    {
        ...
        foreach ( array_reverse( $this->userData->chain->getItems() ) as $chain_item ) {
            $extras_duration = (int) Proxy\ServiceExtras::getTotalDuration( $chain_item->get( 'extras' ) );
    ...
    private function _prepareDates()
    {
        ...
        if ( $this->last_fetched_slot ) {
            // Set timestamp to show next slots as last fetched slots + 1 second
            $this->req_timestamp = $this->last_fetched_slot[0][2] + 1;
    ...
    private function _prepareStaffData()
    {
        ...
        // Load special days.
        $special_days = (array) Proxy\SpecialDays::getDataForAvailableTime( array_keys( $this->staff_data ), $this->start_date, $this->max_date );
    ...
    public function handleCartBookings()
    {
        foreach ( $this->userData->cart->getItems() as $cart_key => $cart_item ) {
            if ( ! in_array( $cart_key, $this->userData->get( 'edit_cart_keys' ) ) ) {
                $extras_duration = (int) Proxy\ServiceExtras::getTotalDuration( $cart_item->get( 'extras' ) );
